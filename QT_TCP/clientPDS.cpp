    ...
    ui(new Ui::ClientPDS)
{
    ...
    setWindowTitle("ClientPDS V1.02");
    ...
    connect(tcpSocket,&QTcpSocket::disconnected,this,
            [=]()
            {
                ui->textEditRead->setText("Disconnect with Host!");
            }
            );
                ...
                if(array[0]=='s'&&array[1]=='t'&&array[2]=='a'&&array[3]=='r')
                {
                        ...
                        switch(command)
                        {
                            ...
                            case PDS_SAFEZONE_CHECK_COMMAND:
                                pds_safezone_check_response_command(array);
                                break;
    ...
    connect(timer, &QTimer::timeout,[=]
                {
                    if(timerEnable)
                    {
                        ...
                        float deltaAngle = ui->DeltaAnglelineEdit->text().toFloat();
                        pdsPalletRequestClass palletRequest(commandID,palletType,deltaAngle);
...
void ClientPDS::timerEvent(QTimerEvent *ev)
{
    if(firstTriger)
    {
        tcpSocket->write(array);
...
void ClientPDS::pds_safezone_check_response_command(QByteArray array)
{
    pdsSafeZoneCheckResponseClass safeZoneCheckResponse(array);
    QString str= "commandID:"+QString::number(safeZoneCheckResponse.safeZoneCheckResponseStruct.commandID) +"\r\n"+
    " errorCode:"+QString::number(safeZoneCheckResponse.safeZoneCheckResponseStruct.errorCode)+"\r\n"+
    " len:"+QString::number(safeZoneCheckResponse.safeZoneCheckResponseStruct.len)+"\r\n"+
    " safezoneExist:"+QString::number(safeZoneCheckResponse.safeZoneCheckResponseStruct.safezoneExist)+"\r\n"+
    " safezone_max_left:"+QString::number(safeZoneCheckResponse.safeZoneCheckResponseStruct.safezone_max_left)+"\r\n"+
    " safezone_max_right:"+QString::number(safeZoneCheckResponse.safeZoneCheckResponseStruct.safezone_max_right)+"\r\n"
            ;
    ui->textEditRead->append(str);
}
...
void ClientPDS::on_pushButtonVolCheck_clicked()
{
//        (command 5)
//    # x for front and back
//    # y for left and right
//    # z for up and down
        QByteArray array;
        pds_point min = {.x=1.0,.y=-0.65,.z=-0.15};
        pds_point max = {.x=2.0,.y=0.65,.z=0};
        pdsVolCheckRequestClass volCheck(min,max);
        array = volCheck.ToArray();
        tcpSocket->write(array);
}
...
void ClientPDS::on_pushButtonSafeZoneCheck_clicked()
{
    QByteArray array;
    pdsSafeZoneCheckRequestClass safeZoneCheck;
    array = safeZoneCheck.ToArray();
    tcpSocket->write(array);
}


void ClientPDS::on_loopButton_clicked()
{
    if(timerEnable == false)
    {
        timerEnable = true;
        ui->loopButton->setText("End Loop");
//        ui->loopButton->setStyleSheet("color:rgb(0,0,0); background-color:rgb(0,255,0)");
        ui->loopButton->setStyleSheet("color:rgb(0,255,0)");
    }
    else
    {
        timerEnable = false;
        ui->loopButton->setText("Begin Loop");
        ui->loopButton->setStyleSheet("color:rgb(255,0,0)");
    }
}
